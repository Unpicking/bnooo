...
class DISCORD_CHECKER:
	...
	def returns(self):
		if self.DV=='pc':print(f"\n{bloFT}┌──(Ban㉿root)-[{wit}~{red}DISCORD-CHECKER.exe{red}{bloFT}]\n└─${wit} The file name is incorrect !\n")
		else:print('\n┌──(Ban㉿root)-[~DISCORD-CHECKER.exe]\n└─$ The file name is incorrect !\n')
	def trts(self):
		if self.Modes=='1':
			self.Mod='File'
			try:
		...
		if self.DV=='pc':print(f"{bloFT}┌──(Ban㉿root)-[{wit}~{red}DISCORD-CHECKER.exe{red}{bloFT}]\n└─${wit} :")
		else:print('┌──(Ban㉿root)-[~DISCORD-CHECKER.exe]\n└─$ :')
...
if __name__ == '__main__':
	OS = input("┌──(Ban㉿root)-[~DISCORD-CHECKER.exe]\n└─$ What are you using ? \n\t\t[1] PC / rdp\n\t\t[2] mobile :")
	if OS == '1':DV='pc';os.system("cls" if os.name=='nt' else "clear");Modes = input(f"{bloFT}┌──(Ban㉿root)-[{wit}~{red}DISCORD-CHECKER.exe{red}{bloFT}]\n└─${wit} {yel}Modes{wit} :\n\t 1- Checking usernames from a file\n\t 2- Automatic checker\n->> Choose the mode: ")
	else:DV='me';os.system('clear');Modes = input(f"┌──(Ban㉿root)-[~DISCORD-CHECKER.exe]\n└─$Modes :\n\t 1- Checking usernames from a file\n\t 2- Automatic checker\n->> Choose the mode: ")
